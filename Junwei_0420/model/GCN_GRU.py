...
class GCN_GRU_singleGCN(torch.nn.Module):
    def __init__(self, in_channels, out_channels, entity_dim, emb_dims, window_size=12):
        super(GCN_GRU_singleGCN, self).__init__()
        self.window_size = window_size
        self.emb_layers = torch.nn.ModuleList([torch.nn.Embedding(x, entity_dim) for x in emb_dims])
        self.gcn1_dict = torch.nn.ModuleDict({str(i):GCNConv(in_channels, out_channels*2)
                                                   for i in range(1)})
    ...
    def forward(self, cat_x, num_x, edges, edge_weights=None):
        ...
        if edge_weights == None:
            gcn_embeddings1 = [self.gcn1_dict[str(0)](x[i], edges[i]).relu() for i in range(self.window_size)]
            gcn_embeddings2 = [self.gcn2_dict[str(0)](gcn_embeddings1[i], edges[i]).unsqueeze(0) for i in range(self.window_size)]
            gcn_embeddings = torch.cat(gcn_embeddings2, 0)
        else:
            gcn_embeddings1 = [self.gcn1_dict[str(0)](x[i], edges[i], edge_weights[i]).relu() for i in range(self.window_size)]
            gcn_embeddings2 = [self.gcn2_dict[str(0)](gcn_embeddings1[i], edges[i], edge_weights[i]).unsqueeze(0) for i in range(self.window_size)]
            gcn_embeddings = torch.cat(gcn_embeddings2, 0)
