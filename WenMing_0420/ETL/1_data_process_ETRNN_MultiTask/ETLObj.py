...
df_cdtx = add_duration_since_20180101(df_cdtx)
gc.collect()
print('add duration since 2018.01.01') 

df_cdtx = add_duration_since_last_trans(df_cdtx)
gc.collect()
...
def extract_cat_num_cols_and_encode_with_catid(df_original, category_cols, numeric_cols):
    df_result = df_original[category_cols + numeric_cols].copy()
    del df_original
    gc.collect()
    df_result.loc[:, category_cols[1:]] = df_result.loc[:, category_cols[1:]].astype(np.str)
    mapper = {col: {value: index+1 for index, value in enumerate(sorted(df_result[col].unique()))} 
          for col in category_cols[1:]}
    df_result[category_cols[1:]] = df_result[category_cols[1:]].apply(lambda x: x.map(mapper[x.name]))
    return df_result, mapper


# extract and organize columns from df_cdtx and save them into df_input 
df_cdtx = feather.read_dataframe('df_cdtx_2.feather')
print('df_cdtx_2 loaded')
...
print('df_full_y_sum_3 loaded')

df_y = extract_target_columns_from_df(df_full_y_sum, ['chid', 'data_dt', 'objam_sum', 'objam_mean', 'trans_count', 'shop_count', 'objam_mean_M3'])
