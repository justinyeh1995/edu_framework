...
class PreProcessor():
    def __init__(self, origin_path, sample_path, tmp_path, result_path, n_chid_sample = None, window_size = 120, test_size = 2):
        origin_path = origin_path   # '../ncku_customer_embedding/data'
        self.sample_path = sample_path # data/sample'
        self.n_sample = n_chid_sample
        self.tmp_path = tmp_path #  'data/tmp'
        self.result_path = result_path # 'data/result'
        self.window_size = window_size 
        self.test_size = test_size 
        self.chid_file = os.path.join(origin_path, 'sample_chid.txt')
        self.cdtx_file = os.path.join(origin_path, 'sample_zip_if_cca_cdtx0001_hist.csv')
        self.cust_f_file = os.path.join(origin_path, 'sample_zip_if_cca_cust_f.csv')

    def connect_pipeline(self):

        chids = Load_chids('load_chids',[], chid_file = self.chid_file)

        sampled_chids = Sample_chids(
            'sample_chids', 
            [chids], 
            result_dir = os.path.join(self.sample_path,'sampled_chids.npy'), 
            n_sample = self.n_sample) 

        chid_to_nid_map = Build_chid_to_nid_map('build_chid_to_nid_map',
                              [sampled_chids],
                              result_dir = os.path.join(self.sample_path,'chid_to_nid_map.npy')
                             )
        ...
        df_full_y_sum = MergeWithOtherTable(
            ...
            result_dir=os.path.join(self.tmp_path, 'df_full_y_sum_2.h5'),
            join_method='inner'
        )
        # df_full_y_sum.run()
        ...
        df_full_y_sum = ConvertTimeCol_into_np_datetime64(
            ...
            [df_full_y_sum],
            result_dir=os.path.join(self.tmp_path, 'df_full_y_sum_3.h5'),
            time_column='data_dt'
        )
        ...
        x_train_x_test_f_train_f_test_y_train_y_test = DataSplit(
            ...
            result_dir=[
                os.path.join(self.tmp_path, 'x_train.npy'),
                os.path.join(self.tmp_path, 'x_test.npy'),
                os.path.join(self.tmp_path, 'f_train.npy'),
                os.path.join(self.tmp_path, 'f_test.npy'),
                os.path.join(self.tmp_path, 'y_train_tmp.npy'),
                os.path.join(self.tmp_path, 'y_test_tmp.npy')
            ],
            window_size=self.window_size, # 120
        ...
        y_train_y_test_y_columns = AddNewTarget_objam_mean_M3_diff(
            ...
            [df_y, y_train_y_test],
            result_dir=[
                os.path.join(self.tmp_path, 'y_train.npy'),
                os.path.join(self.tmp_path, 'y_test.npy'),
                os.path.join(self.tmp_path, 'y_columns.npy')
            ]
        )
        ...
        x_train = SelectResult(
            ...
            [x_train_x_test_f_train_f_test_y_train_y_test],
            selected_indices=[0],
            result_dir=os.path.join(self.result_path, 'x_train.npy')
        )
        ...
        x_test = SelectResult(
            ...
            [x_train_x_test_f_train_f_test_y_train_y_test],
            selected_indices=[1],
            result_dir=os.path.join(self.result_path, 'x_test.npy')
        )
        ...
        f_train = SelectResult(
            ...
            [x_train_x_test_f_train_f_test_y_train_y_test],
            selected_indices=[2],
            result_dir=os.path.join(self.result_path, 'f_train.npy')
        )
        ...
        f_test = SelectResult(
            ...
            [x_train_x_test_f_train_f_test_y_train_y_test],
            selected_indices=[3],
            result_dir=os.path.join(self.result_path, 'f_test.npy')
        )
        ...
        y_train = SelectResult(
            ...
            [y_train_y_test_y_columns],
            selected_indices=[0],
            result_dir=os.path.join(self.result_path, 'y_train.npy')
        )
        ...
        y_test = SelectResult(
            ...
            [y_train_y_test_y_columns],
            selected_indices=[1],
            result_dir=os.path.join(self.result_path, 'y_test.npy')
        )

        columns = XFYColumnBuilder('create_x_f_y_columns',
                                   [df_input, df_feat_input, y_columns],
                                   result_dir=os.path.join(self.result_path, 'columns.npy')
                                   )
        ...
        feature_map = SelectResult(
            ...
            [df_input_feature_map],
            selected_indices=[1],
            result_dir=os.path.join(self.result_path, 'feature_map.npy')
        )
        ...
        cust_feature_map = SelectResult(
            ...
            [df_feat_input_cust_feature_map],
            selected_indices=[1],
            result_dir=os.path.join(self.result_path, 'cust_feature_map.npy')
        )

        return x_train, x_test, y_train, y_test, feature_map, chid_to_nid_map, columns

class Load_chids(ETLPro):
    def __init__(self, process_name, pre_request_etls, result_dir=None, chid_file = None):
        assert chid_file 
        super(Load_chids, self).__init__(process_name, pre_request_etls, result_dir=result_dir)
        self.chid_file = chid_file
    def process(self, inputs):
        chid_array = np.loadtxt(self.chid_file, dtype=np.str)
        return [chid_array]
...
class Load_cdtx(ETLPro):
    def __init__(self, process_name, pre_request_etls, result_dir=None, cdtx_file = None):
        assert cdtx_file 
        super(Load_cdtx, self).__init__(process_name, pre_request_etls, result_dir=result_dir)
        self.cdtx_file = cdtx_file
    def process(self, inputs):
        chids = inputs[0]
        t0 = time()
        df_cdtx = pd.read_csv(self.cdtx_file, skipinitialspace=True)
...
class Load_cust_f(ETLPro):
    def __init__(self, process_name, pre_request_etls, result_dir=None, cust_f_file = None):
        assert cust_f_file 
        super(Load_cust_f, self).__init__(process_name, pre_request_etls, result_dir=result_dir)
        self.cust_f_file = cust_f_file
    def process(self, inputs):
        chids = inputs[0]
        t0 = time()
        df_cust_f = pd.read_csv(self.cust_f_file, skipinitialspace=True)
